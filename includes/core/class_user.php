...
class User {
    ...
    public static function user_update($data) {
        ...
        if (!$set) return 'Set is empty';
        // query
        DB::query("UPDATE users SET ".implode(', ', $set)." WHERE user_id='".$user_id."'") or die (DB::error());
        Notification::notification_create(['user_id' => $user_id, 'title' => 'User data updated', 'description' => 'User data updated']);
        //info
        return User::user_info(['user_id' => $user_id]);
    }

    // TEST

    public static function owner_info() {
        return User::user_info(['user_id' => Session::$user_id]);
    }

    public static function owner_update($data = []) {
        $data['user_id'] = Session::$user_id;
        return User::user_update($data);
    }
}
